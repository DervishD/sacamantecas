...
class BaseParser(HTMLParser):
    ...
    def is_parser_for_profile(cls, profile):
        ...
        the same as the profile.keys(), excluding URL_REGEX_KEY as it is unused
        in the parsers but present in all of the profiles anyway.
        """
        return (cls.NEEDED_KEYS | {BaseParser.URL_REGEX_KEY}) == profile.keys()
...
def retrieve_url(url):
    ...
    First resolve meta-refresh redirection for URL, then get the contents and
    ...
    if url.startswith('file://'):
        url = urlparse(url)
        if not url.netloc:
            # Remember that url.path ALWAYS starts with '/', must be ignored.
            url = url._replace(path=quote(str(Path(unquote(url.path[1:])).resolve().as_posix()), safe=':/'))
        url = urlunparse(url)
    try:
        with urlopen(Request(url, headers={'User-Agent': USER_AGENT})) as request:
            ...
            if match:
                url = urlparse(url)
                url = urlunparse((url.scheme, url.netloc, match.group(1).decode('ascii'), '', '', ''))
                logging.debug('Redirección -> «%s».', url)
                with urlopen(Request(url, headers={'User-Agent': USER_AGENT})) as redirected_request:
                    contents = redirected_request.read()
                    charset = redirected_request.headers.get_content_charset()
            else:
                logging.debug('El URL no está redirigido.')
